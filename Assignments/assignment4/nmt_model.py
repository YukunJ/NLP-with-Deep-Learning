...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 1) Tensor 'X' of shape (src_len, b, e)
        X = self.model_embeddings.source(source_padded)
        # 2) Pack dense -> decoder -> Pad back -> switch dim
        ...
        # 3) dec_init_state = (init_decoder_hidden, init_decoder_cell)
          # -init_decoder_hidden
        last_cat = torch.cat((last_hidden[0], last_hidden[1]), 1)
        init_decoder_hidden = self.h_projection(last_cat)
          #) -init_decoder_cell
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            # - concat y_t with o_prev
            ybar_t = torch.cat((y_t, o_prev), 1)
            # - use step function decode forward
        ...
        # 4) stack lst of (b, h) -> (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 1) run decoder to move forward
        dec_state = self.decoder(Ybar_t, dec_state)
        # 2) split into hidden and cell
        dec_hidden, dec_cell = dec_state
        # 3) attention score compute
          # - unsqueeze

...
def question_1e_sanity_check():
    ...
    assert(small_ind == small_ind_gold), \
        "small test resulted in indices list {:}, expected {:}".format(
            small_ind, small_ind_gold)

    print('Running test on large list of sentences')
    tgt_sents = [['<s>', "Let's", 'start', 'by', 'thinking', 'about', 'the', 'member', 'countries', 'of', 'the', 'OECD,', 'or', 'the', 'Organization', 'of', 'Economic', 'Cooperation', 'and', 'Development.', '</s>'], ['<s>', 'In', 'the', 'case', 'of', 'gun', 'control,', 'we', 'really', 'underestimated', 'our', 'opponents.',
                                                                                                                                                                                                                         '</s>'], ['<s>', 'Let', 'me', 'share', 'with', 'those', 'of', 'you', 'here', 'in', 'the', 'first', 'row.', '</s>'], ['<s>', 'It', 'suggests', 'that', 'we', 'care', 'about', 'the', 'fight,', 'about', 'the', 'challenge.', '</s>'], ['<s>', 'A', 'lot', 'of', 'numbers', 'there.', 'A', 'lot', 'of', 'numbers.', '</s>']]
    tgt_ind = vocab.words2charindices(tgt_sents)
    tgt_ind_gold = pickle.load(
        open('./sanity_check_en_es_data/1e_tgt.pkl', 'rb'))
    assert(tgt_ind == tgt_ind_gold), "target vocab test resulted in indices list {:}, expected {:}".format(
...
def model_embedding_sanity_checks():
    """ Sanity checks for the `Model_Embedding` module. i-j
    """
    print("-"*80)
    ...
    # Test CNN output shapes *without maxpool* - comment out the maxpool in
    # `forward()`.
    # cnn = CNN(input_channel_count=E_CHAR, output_channel_count=E_WORD)
    # output = cnn(input)
    ...
    # assert output.shape[2] == M_WORD - K + \
    #     1, "Wrong length size (length dim). {} vs {}".format(output.shape[2], M_WORD - K + 1)

    # Test CNN output shapes *with maxpool*
    cnn = CNN(input_channel_count=E_CHAR, output_channel_count=E_WORD)
    output = cnn(input)
    assert output.shape[0] == SRC_LEN * \
        B, "output shape is incorrect: it should be:\n {} but is: \n {}".format(
    ...
    # Sanity check for the `Highway` module.
    ...
    highway = Highway(word_embed_size=EMBED_SIZE)

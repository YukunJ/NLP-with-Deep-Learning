...
class VocabEntry(object):
    ...
    def char_count(self):
        """
        return the number of characters in VocabEntry
        @returns len (int) : number of characters in the VocabEntry
        """
    ...
    def words2charindices(self, sents):
        ...
        # prepend & append the character indice for the start_character '{' and end_character '}'
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_var = torch.tensor(pad_sents_char(word_ids, self.char2id['<pad>']), dtype=torch.long, device=device)

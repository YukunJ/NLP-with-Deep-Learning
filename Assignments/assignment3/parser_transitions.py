...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###         3. Right Arc
        if transition == "S":
            # pop from the buffer and append into stack
            shift = self.buffer.pop(0)
            self.stack.append(shift)
        elif transition == "LA":
            head = self.stack.pop() # the stack top
            dependent = self.stack.pop() # the second in stack
            self.dependencies.append((head,dependent))
            self.stack.append(head) # bring the head back into stack
        elif transition == "RA":
